...
export class GrForklift extends GrObject {
  ...
  constructor(params = {}) {
    ...
    cab_group.translateY(0);
    let cab_curve = new T.Shape();
    cab_curve.moveTo(1, 0);
    cab_curve.lineTo(-1, 0);
    cab_curve.lineTo(-0.8, 0.75);
    cab_curve.lineTo(-0.25, 0.75);
    cab_curve.lineTo(0, 1.2);
    cab_curve.lineTo(0.8, 1.2);
    cab_curve.lineTo(1, 0.9);
    cab_curve.lineTo(1, 0);
    ...
    let arm_translation_group = new T.Group();
    arm_group.add(arm_translation_group);
    
    // create two boxes for the arms of the forklift
    let arm_geom = new T.BoxGeometry(1, 0.2, 0.2);
    ...
    let arm1 = new T.Mesh(arm_geom, arm_mat);
    arm_translation_group.add(arm1);
    arm1.translateZ(-0.2);
    arm1.translateY(0.4);
    arm1.translateX(-0.5);
    ...
    let arm2 = new T.Mesh(arm_geom, arm_mat);
    arm_translation_group.add(arm2);
    arm2.translateZ(0.2);
    arm2.translateY(0.4);
    arm2.translateX(-0.5);
    ...
    // create two boxes for the holder of the arms of the forklift
    let holder_geom = new T.BoxGeometry(0.2, 0.2, 3);
    let holder1 = new T.Mesh(holder_geom, arm_mat);
    holder1.rotateX(Math.PI / 2);
    holder1.translateZ(-1);
    holder1.translateY(0.2);
    holder1.receiveShadow = true;
    holder1.castShadow = true;
    arm_group.add(holder1);

    let holder2 = new T.Mesh(holder_geom, arm_mat);
    holder2.rotateX(Math.PI / 2);
    holder2.translateZ(-1);
    holder2.translateY(-0.2);
    holder2.receiveShadow = true;
    holder2.castShadow = true;
    arm_group.add(holder2);

    // add driver's roof
    let pillar_geom = new T.BoxGeometry(0.1, 0.1, 1.8);
    let pillar_mat = new T.MeshStandardMaterial({
      color: "gray",
      metalness: 0.6,
      roughness: 0.3
    });
    let lfpillar = new T.Mesh(pillar_geom, pillar_mat);
    lfpillar.rotation.set(Math.PI/2, -Math.PI/20, 0);
    lfpillar.position.set(0, 1.8, 0.25);
    lfpillar.receiveShadow = true;
    lfpillar.castShadow = true;
    cab_group.add(lfpillar);
    let rfpillar = new T.Mesh(pillar_geom, pillar_mat);
    rfpillar.rotation.set(Math.PI/2, -Math.PI/20, 0);
    rfpillar.position.set(0, 1.8, -0.25);
    rfpillar.receiveShadow = true;
    rfpillar.castShadow = true;
    cab_group.add(rfpillar);
    let lbpillar = new T.Mesh(pillar_geom, pillar_mat);
    lbpillar.rotation.set(Math.PI/2, 0, 0);
    lbpillar.position.set(1, 1.8, 0.25);
    lbpillar.receiveShadow = true;
    lbpillar.castShadow = true;
    cab_group.add(lbpillar);
    let rbpillar = new T.Mesh(pillar_geom, pillar_mat);
    rbpillar.rotation.set(Math.PI/2, 0, 0);
    rbpillar.position.set(1, 1.8, -0.25);
    rbpillar.receiveShadow = true;
    rbpillar.castShadow = true;
    cab_group.add(rbpillar);

    let roof_geom = new T.BoxGeometry(1, 0.1, 0.75);
    let roof = new T.Mesh(roof_geom, excavator_mat);
    roof.position.set(0.55, 2.75, 0);
    roof.receiveShadow = true;
    roof.castShadow = true;
    cab_group.add(roof);

    // add cylinders for wheels
    let wheel_geom = new T.CylinderGeometry(0.2, 0.2, 0.2, 32);
    ...
    wheel1.translateY(0.2);
    wheel1.translateZ(0.5);
    ...
    wheel3.translateY(0.2);
    wheel3.translateZ(0.5);
    ...
    wheel4.translateY(0.2);
    wheel4.translateZ(-0.5);
    ...
    this.cab = cab_group;
    this.arm = arm_translation_group;
  ...
  update(paramValues) {
    ...
    this.arm.position.y = degreesToRadians(paramValues[3]);
  }
}
...
export class GrBulldozer extends GrObject {
  ...
  constructor(params = {}) {
    ...
    // create two boxes for the holder of the digger
    let holder_geom = new T.BoxGeometry(0.2, 0.2, 3);
    let holder1 = new T.Mesh(holder_geom, arm_mat);
    holder1.rotateY(Math.PI / 2);
    holder1.translateX(-0.5);
    holder1.translateY(0.2);
    holder1.translateZ(-1.1);
    holder1.receiveShadow = true;
    holder1.castShadow = true;
    arm_group.add(holder1);
    ...
    holder2.translateX(0.5);
    holder2.translateY(0.2);
    holder2.translateZ(-1.1);
    holder2.receiveShadow = true;
    holder2.castShadow = true;
    arm_group.add(holder2);
    ...
    let arm_translation_group = new T.Group();
    arm_group.add(arm_translation_group);
    arm_translation_group.translateY(0.2);
    arm_translation_group.translateX(-2.5);
    
    // create a box for the digger
    let digger_geom = new T.BoxGeometry(0.1, 0.5, 1.25);

    let digger1 = new T.Mesh(digger_geom, arm_mat);
    digger1.position.set(-0.1, 0, 0);
    digger1.receiveShadow = true;
    digger1.castShadow = true;

    let digger_up = new T.Mesh(digger_geom, arm_mat);
    digger_up.position.set(-0.25, 0.4, 0);
    digger_up.rotateZ(Math.PI / 4);
    digger_up.receiveShadow = true;
    digger_up.castShadow = true;

    let digger_down = new T.Mesh(digger_geom, arm_mat);
    digger_down.position.set(-0.25, -0.4, 0);
    digger_down.rotateZ(-Math.PI / 4);
    digger_down.receiveShadow = true;
    digger_down.castShadow = true;

    let trapezoid_geom = new T.CylinderGeometry(0.3, 0.62, 0.4, 4, 1 );
    let digger_left = new T.Mesh(trapezoid_geom, arm_mat);
    digger_left.position.set(-0.2, 0, 0.6);
    digger_left.scale.set(1, 1, 0.5);
    digger_left.rotateZ(-Math.PI / 2);
    digger_left.receiveShadow = true;
    digger_left.castShadow = true;

    let digger_right = new T.Mesh(trapezoid_geom, arm_mat);
    digger_right.position.set(-0.2, 0, -0.6);
    digger_right.scale.set(1, 1, 0.5);
    digger_right.rotateZ(-Math.PI / 2);
    digger_right.receiveShadow = true;
    digger_right.castShadow = true;

    arm_translation_group.add(digger1);
    arm_translation_group.add(digger_up);
    arm_translation_group.add(digger_down);
    arm_translation_group.add(digger_left);
    arm_translation_group.add(digger_right);

    // add tubes for wheels
    let wheel_geom = new T.TorusGeometry(0.5, 0.2);
    let wheel_mat = new T.MeshStandardMaterial({
      color: "black",
      metalness: 0.6,
      roughness: 0.3
    ...
    this.cab = cab_group;
    this.arm = arm_group;
    this.digger = arm_translation_group;
